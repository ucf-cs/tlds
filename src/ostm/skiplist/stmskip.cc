 ...
 */
static node_t *search_predecessors(ptst_t *ptst, stm_tx *tx, set_t *l,
                                   setkey_t k, stm_blk **pa, stm_blk **na) {
  stm_blk *xb, *x_nextb;
  node_t *x, *x_next;
  int i;
...
set_t *stmskip_alloc(void) {
  ...
  h->k = SENTINEL_KEYMIN;
  h->level = NUM_LEVELS;
  for (i = 0; i < NUM_LEVELS; i++) h->next[i] = tb;
...
setval_t stmskip_update(ptst_t *ptst, stm_tx *tx, set_t *l, setkey_t k,
                        setval_t v) {
  setval_t ov;
...
stmskip_execute_ops(void *s, set_op ops[], int op_size) {
  set_t *l = (set_t *)s;
...
void destory_stmskip_subsystem(void) {
  ...
  printf("Total commit %u, abort (total/fake) %u/%u\n", g_count_commit,
         g_count_abort, g_count_abort - g_count_real_abort);
}
