...
// NOTE: this was originally c code, so we pass a pointer to our transskip, l,
// since there is no transskip class
...
static inline bool FinishPendingTxn(trans_skip* l, NodeDesc* nodeDesc,
                                    Desc* desc) {
 ...
 */
static node_t* strong_search_predecessors(trans_skip* l, setkey_t k,
                                          node_t** pa, node_t** na) {
...
static node_t* weak_search_predecessors(trans_skip* l, setkey_t k, node_t** pa,
                                        node_t** na) {
  node_t *x, *x_next;
  setkey_t x_next_k;
  int i;
...
bool transskip_delete(trans_skip* l, setkey_t k, Desc* desc, uint8_t opid,
                      node_t*& n) {
  n = NULL;
  bool ret = false;
...
void destroy_transskip_subsystem(void) { fr_destroy_gc_subsystem(); }
