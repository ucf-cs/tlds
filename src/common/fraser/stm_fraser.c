...
#define ARENA_SIZE      128880
#define DESCRIPTOR_SIZE  12888
...
static void release_descriptor(ptst_t *ptst, stm_tx *t)
{
    ...
    if ( t->status == TXS_SUCCESSFUL )
    {
        ...
        {
            fr_gc_free(ptst, ent->old, t->gc_data_id);
        }
    }
    else
    {
        for ( ent = t->writes; ent != NULL; ent = ent->next )
        {
            fr_gc_unsafe_free(ptst, ent->new, t->gc_data_id);
        }
    }    
}
...
stm *new_stm(ptst_t *ptst, int blk_size)
{
    ...
    mem->gc_data_id = fr_gc_add_allocator(ALLOCATOR_SIZE(blk_size));
    return mem;
}
...
{
    fr_gc_remove_allocator(mem->gc_data_id);
    free(mem);
}
...
{
    stm_blk *b;
    b       = fr_gc_alloc(ptst, gc_blk_id);
    b->data = fr_gc_alloc(ptst, mem->gc_data_id);
    return b;
}
...
void free_stm_blk(ptst_t *ptst, stm *mem, stm_blk *b)
{
    ...
    fr_gc_free(ptst, data, mem->gc_data_id);
    fr_gc_free(ptst, b,    gc_blk_id);
}
...
bool_t commit_stm_tx(ptst_t *ptst, stm_tx *t)
{
 ...
 fail:
    if ( read_only(t) )
    {
        ...
        return t->status == TXS_SUCCESSFUL;
    }
...
void *write_stm_blk(ptst_t *ptst, stm_tx *t, stm_blk *b)
{
    ...
    ent->new  = fr_gc_alloc(ptst, t->gc_data_id);
    ent->next = *w_pent;
    *w_pent = ent;
...
void remove_from_tx(ptst_t *ptst, stm_tx *t, stm_blk *b)
{
    ...
    { 
        *pent = ent->next;
        data = ent->new;
        assert(!is_descriptor(data));
        fr_gc_free(ptst, data, t->gc_data_id);
        return; 
    }
...
void _init_stm_subsystem(int pad_data)
{
    ...
    do_padding = pad_data;
    gc_blk_id = fr_gc_add_allocator(ALLOCATOR_SIZE(sizeof(stm_blk)));
