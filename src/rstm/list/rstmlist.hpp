...
RSTMList::RSTMList() : sentinel(new Node()) { }
...
bool RSTMList::extendedSanityCheck(verifier v, uint32_t v_param) const
{
    const Node* prev(sentinel);
    const Node* curr((prev->m_next));
    while (curr != NULL) {
...
TM_CALLABLE
bool RSTMList::insert(int val TM_ARG)
{
...
// search function
TM_CALLABLE
bool RSTMList::lookup(int val TM_ARG) const
{
    bool found = false;
...
// findmax function
TM_CALLABLE
int RSTMList::findmax(TM_ARG_ALONE) const
{
    int max = -1;
    const Node* curr(sentinel);
    while (curr != NULL) {
...
// findmin function
TM_CALLABLE
int RSTMList::findmin(TM_ARG_ALONE) const
{
    int min = -1;
    const Node* curr(sentinel);
    curr = TM_READ(curr->m_next);
...
// remove a node if its value == val
TM_CALLABLE
bool RSTMList::remove(int val TM_ARG)
{
    bool ret = false;
...
// search function
TM_CALLABLE
void RSTMList::overwrite(int val TM_ARG)
{
    const Node* curr(sentinel);
    curr = TM_READ(curr->m_next);
