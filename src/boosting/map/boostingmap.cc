...
BoostingMap::ReturnCode BoostingMap::Insert(uint32_t key, uint32_t val, uint32_t threadId)
{
    if(!m_lock.Lock(key))
    {
        return LOCK_FAIL;
...
BoostingMap::ReturnCode BoostingMap::Delete(uint32_t key, uint32_t threadId)
{
    ...
    // before we remove the node, and while its still locked, we save val in case the operation succeeds, but the transaction fails
...
BoostingMap::ReturnCode BoostingMap::Find(uint32_t key, uint32_t threadId)
{
    ...
    // Nothing to undo if a find commits, but the transaction fails, so add nothing to the log
    return OK;
}

BoostingMap::ReturnCode BoostingMap::Update(uint32_t key, uint32_t expected, uint32_t val, uint32_t threadId)
{
    if(!m_lock.Lock(key))
    {
        return LOCK_FAIL;
    ...
    // To get here the update must have been successful, which means the expected value was there originally,
    // and val is there now. So, "swap" them to get back to the original state, if the transaction fails.
