...
class WaitFreeHashTable {
 public:
  ...
  WaitFreeHashTable(int initalPowerOfTwo, int numThreads) {
    ...
    // note: MAIN_SIZE needs to be the nearest power of two to the number of
    // nodes that is passed in as the "initialPowerOfTwo"
    MAIN_SIZE =
        WaitFreeHashTable::POW(((int)std::ceil(std::log2(initalPowerOfTwo))));
...
// v_capacity=MAIN_SIZE;
  ...
  bool putUpdate(KEY k, VALUE e_value, VALUE n_value, int T) {
    return putUpdate_first(k, e_value, n_value, T);
  };
  bool putIfAbsent(KEY k, VALUE v, int T) {
    return putIfAbsent_first(k, v, T);
  };
  ...
  // SUB_POW,elements); 	void */* volatile  */*local=head;
  ...
  // 		}
  // 	}
  // 	printf("On Main Spine: Data Nodes:%d\t Spine Nodes: %d\t
  ...
  // 			if(isSpine(node)){
  // 				int*
  // c_res=count_nodes_at_depth(unmark_spine(node),1,j);
  ...
  // 			}
  // 		}
  // 		printf("On Depth %d: Data Nodes:%d\t Spine Nodes: %d\t
  ...
  // 				int*
  // c_res=count_nodes_at_depth(unmark_spine(node),d+1,t_d); 				int p=0;
  // 				count[p]=count[p]+c_res[p++];
  // 				count[p]=count[p]+c_res[p++];
  // 				count[p]=count[p]+c_res[p++];
  // 			}
  ...
  // 		}
  // 	}
  // 	printf("Reusable data nodes: %d (total mem uses
  ...
  // 		}
  // 	}
  // 	printf("Reusable Spine nodes: %d (total mem uses
 ...
 private:
  ...
  Significant Node is the position in the hash table where the key belongs. This
  has a direct relationship to depth.
  ...
  If the Significant node is read as NULL the thread will CAS NULL for its Node;
          ...
          If the CAS fails and the Significant is now
                  ...
                  *A key match, it returns, it linearizes that the key's value
  was updated then immeditly replaced by a newwer value. *A non key match, it
  attempts to expand and insert. If expansion fails, it examines the significant
  node again, incrementing its cas_fail_count If it Succedes then it returns. *A
  Spine node, then it examines the next significant node and acts accordingly.

  If the Significant node is read as a Key Match the thread will CAS the Key
  Match for its Node; If the CAS passes, the thread will return. If the CAS
  fails and the Significant is now *NULL, returns, it linearizes that the key's
  value was updated then immeditly deleted by another thread *A key match, it
  returns, it linearizes that the key's value was updated then immeditly
  replaced by a newwer value. *A non key match: For this to occur a thread must
  delete the key, and a thread must insert a different key at this position. It
  linearizes that the key's value was updated then immeditly deleted by another
  thread, then another thread inserted a non key match *A Spine node, then it
  examines the next significant node and acts accordingly.

  If the Significant node is read as a NON Key Match the thread will CAS the NON
  Key Match for a Spine containing the threads node and the current node, and
  possibly additional spines that resolve further collision; If the CAS passes,
  the thread will return. If the CAS fails and the Significant is now *NULL, it
  will procede with the logic above for reading NULL value, incrementing its
  cas_fail_count *A key match, it returns, it linearizes that the non key match
  was deleted, then the thread inserted its value which was then immeditly
  replaced by a newwer value. *A non key match, it will try again, incrementing
  its cas_fail_count. *A Spine node, then it examines the next significant node
  ...
  If the Significant node is read as spine node, then the thread will
          ...
          3) Isolate the position of the significant  node from the bits of the
  shifted hash 4)  Read the significant node and act acodingly.

  If in the rare and unlikely case the cas_fail_count is greater then the
  defined threshold, the thread will use atmoic sync fetch and or to mark the
  significant node. If a node is marked then the thread will CAS that node for a
  Spine contating that node. The reason for this is that all operations must be
  bounded in a Wait-Free Algorithm, regardless of CAS contention. This bounds
  the number of times the thread will loop as described above. Once a node is
  marked, it must be replaced by a spine node.

  The number of spines that can be made is bounded by the constants KeySize, M
  and S. (Keysize=number of bits in the key) (2^M=Length of the Main Memory
  Array) (2^S=Length of the Sub Memory Arrays) The Max Depth is equal to
  1+(KeySize-M)/S rounded up
  ...
  */
  ...
  inline bool putIfAbsent_first(KEY k, VALUE v,
                                int T) {  // T is the executing thread's ID
    HASH hash = HASH_KEY(
...
// Allocates a bucket, then stores the value, key and hash into it
  ...
  */
  inline bool putIfAbsent_sub(void * /* volatile  */ *local,
  ...
  inline bool putUpdate_first(KEY k, VALUE e_value, VALUE v,
                              int T) {  // T is the executing thread's ID
    if (e_value == v) return true;
  ...
  inline bool putUpdate_main(HASH hash, VALUE e_value, DataNode *temp_bucket,
                             int T) {
    // This count bounds the number of times the thread will loop as a result of
    // CAS failure.
    int cas_fail_count = 0;
      ...
      if (node == NULL) {  // See Logic Above
        ...
        if (((DataNode *)node)->hash ==
          ...
          if (((DataNode *)node)->value != e_value) {
            ...
            void *node2;
            if ((node2 = replace_node(head, pos, node, temp_bucket)) ==
                node) {  // Attempt to replace the node
              // Pass the CAS
              Free_Node(node, T);  // Frees the original
              return true;
            } else {  // Failed the CAS
              if (node2 ==
                  NULL) {  // If it is NULL the return (see above for why)
                return false;
              } else if (isSpine(
                             node2)) {  // If it is a Spine then continue, we
                                        // don't know if the key was updated.
                return putUpdate_sub(unmark_spine(node2), e_value, temp_bucket,
                                     T);
              }
...
#endif
                node = forceExpandTable(
                    T, head, pos, unmark_data(node2),
                    MAIN_POW);  // The expanded spine is returned
                return putUpdate_sub(unmark_spine(node), e_value, temp_bucket,
                                     T);  // We Examine the next level
              } else {
                return false;
              }
            }
   ...
   **DONT FORGET: to do get/delete as well
   */
  inline bool putUpdate_sub(void * /* volatile  */ *local, VALUE e_value,
                            DataNode *temp_bucket, int T) {
    HASH h = (temp_bucket->hash) >>
             MAIN_POW;  // Shifts the hash to move the siginifcant bits to the
                        // right most position
        ...
        if (node == NULL) {  // Same logic as Put Main
          ...
          if (((DataNode *)node)->hash ==
            ...
            if ((node2 = replace_node(local, pos, node, temp_bucket)) == node) {
              Free_Node(node, T);  // CAS Succedded, no need to update size as
                                   // we are only replacing
              return true;
            } else {
...
#endif
                node = forceExpandTable(
                    T, local, pos, unmark_data(node2),
                    right + SUB_POW);  // The expanded spine is returned
                local = unmark_spine(node);
                break;
              }
              // We can linearize that the value was inserted then imeditly
              // replace in any olther cas
              else {
                return false;
              }
            }
  ...
  /**
  This function is used to expand the table at a position that is under heavy
  CAS contention. IF a thread fails to pass a CAS on a position in X tries, and
  the current values after those CAS operations don't allow the thread to
  return, the thread will mark the node.
  ...
  If we mark a spine node by mistake there is no issues because the check
  isSpine only considers a single bit that is a different bit then the is marked
  data. Further the unmarkspine function clears both bits.
...
#endif
    VALUE v = get_main(h);  // Calls the get main function							//TODO:
                            // MemCopy?
#ifdef useThreadWatch
    Thread_watch[T] = 0;  // Removes the hash from the watchlist
#endif
  ...
  **/
  ...
  inline bool remove_main(HASH hash, int T) {
    ...
    if (((DataNode *)node)->hash == hash) {  // HASH COMPARE
      if (replace_node(head, pos, node)) {   // Tries to CAS the key match to
                                            // NULL
        ...
        if (isMarkedData(node2) &&
            unmark_data(node2) !=
                node) {  // If it is the same node but marked is at the location
          // Then expand the table, and examine the sub spine
          node2 = forceExpandTable(
              T, head, pos, node,
              MAIN_POW);  // getNodeRaw must return a spine pointer
      ...
      if (isMarkedData(node)) {  // If it is marked then force expand, then
        ...
        if (((DataNode *)node)->hash == hash) {  // HASH COMPARE
          if (replace_node(local, pos, node)) {  // Try to remove
            ...
            if (isMarkedData(node2) &&
                unmark_data(node2) !=
                    node) {  // If it is the same node but marked then force
                             // expand and examine subspine
...
**/
  ...
  inline DataNode *Allocate_Node(VALUE v, HASH h, int T) {
    ...
    if (new_temp_node != NULL) {  // Check to see if the stack was empty
      ...
      // printf("\n\nvalue: %d hash: %d --- %llu < 200\n\n", v, h,
      // new_temp_node); fflush(stdout); goto elsestmt;
      // }
...
#ifdef useThreadWatch
    if (!inUse(((DataNode *)node)->hash,
               T))  // If it is Not in use, then place it on the stack
    {
      // Add to Stack
      ((DataNode *)node)->next =
...
#endif
          return;
        } else if (!inUse(D->hash, T)) {  // If it is not inuse move the node to
                                          // the stack, then replace it
      ...
      Thread_pool_vector[T] =
          (void **)realloc(Thread_pool_vector[T], sizeof(void *) * (size + 1));
...
//////////////////////////////////////////////////////////////////////////////////
  ...
  void check_table_state_p() {
    ...
    for (int i = 0; i < MAIN_SIZE; i++) {
      ...
      if (i != pos) {
        printf("Item out of place on Main:at POS %d, belongs in %d, hash: %d, ",
               i, pos, sig_node->hash);
  ...
  int check_table_state(void * /* volatile  */ *s, int right, int d) {
    ...
    for (int i = 0; i < SUB_SIZE; i++) {
      ...
      if (!check_key(d, sig_node->hash)) {
        ...
        print_key(sig_node->hash);
        printf("Traversal: ");
        for (int j = 0; j <= d; j++) printf("%3d -", depth[j]);
        printf("\n");
        printf("Node: K:%u HASH_KEY(%u) V:%lu H:%u \t", 0 /*sig_node->key*/,
               0 /*HASH_KEY(sig_node->key)*/, sig_node->value, sig_node->hash);
#ifdef USE_KEY
  ...
  void quick_print(void * /* volatile  */ *s, int l, bool recur, int tab) {
    for (int i = 0; i < l; i++) {
      ...
      if (isSpine(n)) {
        printf(" SPINE NODE \n");
        if (recur) quick_print(s, SUB_SIZE, true, tab + 1);
      } else {
