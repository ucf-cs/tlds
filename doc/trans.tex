...
\label{sec:related}
golan2015automatic/golan2013/bronson2010transactional
Transactional execution of code 
...
However, as many researchers later realize, applying its synchronization model based on detecting read/write conflicts to lock-free data structures causes false conflict~\cite{} and excessive aborts under heavy contention~\cite{christina2015resource}.
Thus, considerable amount of work and ingenuity has gone into designing lock-free data structures using low-level synchronization primitives such as \texttt{CompareAndSwap}, which empowers researchers to devise algorithm-specific fine-grained concurrency control schemes.
Thus, considerable amount of work and ingenuity has gone into designing lock-free data structures using low-level synchronization primitives such as \texttt{CompareAndSwap}, which gives programmer fine-grained control over 
...
\subsection{Transactional Boosting}
