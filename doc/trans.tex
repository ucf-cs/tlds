...
\begin{algorithm}[t]
	...
	\KwRet{ retry}
	%endif
	$\textbf{if}$  {$oldinfo.desc \neq info.desc$} $\textbf{then}$
	$CAS(\&oldInfo.desc,\;Active,\;Aborted)$ \label{l:abortcompetitor}
	$textbf{else if}$  {$oldinfo.opid \ge info.opid$} $\textbf{then}$
	\KwRet{ success}
	%endif
	$\textbf{bool}\;haskey \gets IsKeyPresent(oldinfo)$
	$\textbf{if}$ { $(!haskey \;\textbf{and}\; wantkey) \;\textbf{or}\; (haskey \;\textbf{and}\; !wantkey)$} } $\textbf{then}$
\KwRet{ fail }
%endif
$\textbf{if}$ { {$info.desc.status \neq Active$} $\textbf{then}$
	\KwRet{ fail }
	%endif
	$\textbf{if}$  {$CAS(\&n.info,\;oldinfo,\;info)$}  $\textbf{then}$
	\KwRet{ success}
	$textbf{else}$
	\KwRet{ retry}
	%endif
...
\begin{algorithm}[th]
	...
	$\textbf{if}$ { {$ret = \textbf{true}$}} $\textbf{then}$
	$\textbf{if}$ { {$\text{\textsc{CAS}}(\&desc.flag,\;Active,\;Committed)$} } $\textbf{then}$
