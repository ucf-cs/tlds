...
This function returns whether or not the value present in the \textsc{Node} should be treated as present; if not, the value in the \textsc{NodeInfo} descriptor is used, because there is a pending update from the same transaction descriptor.
In the \textsc{IsValuePresent} function, \textsc{INVALID} represents a sentinel value that indicates that a value has not been set for a \textsc{Find} operation.
...
\begin{algorithm}[t]
	...
	\Function{\IsValuePresent{$\textbf{NodeInfo*}\;oldInfo,\;\textbf{NodeInfo*}\;newInfo$}}{
%		$\textbf{TxStatus}\;status \gets info.desc.status$ \;
		
		$\textbf{Operation}\;op\gets oldInfo.desc.ops[oldInfo.opid]$\;
		\If{$op.type == Update\; ||\; op.type == Find \;\&\&\; op.value != \text{INVALID}$}{
			$newInfo.desc.ops[newInfo.opid].value \gets op.value$\;
			$return\;false$\;
		}
		\Else{
			$return\;true$\;
...
%	(nodeDesc->desc->ops[nodeDesc->opid].type == MAP\_FIND && nodeDesc->desc->ops[nodeDesc->opid].value != 0) )//nodeDesc->value != 0) )
%	return true;
%	return false;
%}
%
