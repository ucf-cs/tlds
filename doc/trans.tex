...
% Page heads
\markboth{D. Zhang et al.}{Lock-free Transactions without Rollbacks for Linked Data Structures}
...
\begin{algorithm}[t]
	...
	\Function{\Insert{$\textbf{int}\;key,\;\textbf{Desc*}\;desc,\;\textbf{int}\;opid$}}{
    	...
    	\While{\textbf{true}}{ \label{l:listinsertwhile}
    		...
    		\If{IsNodePresent($curr,\;key$)}{ \label{l:listinsertcontain}
...
\begin{algorithm}[h]
	...
	\Function{\MarkDelete{$\textbf{set}\;delnodes,\;\textbf{Desc*}\;desc$}}{
		\For {$del \in delnodes$}{
			...
			\If{$CAS(del.info,info,\textsc{SetMark}(info))$}{ \label{l:markinfo}
				$Do\_Delete(del)$
			}
		}
	}
...
We propagate this change to the logical status interpretation algorithm by treating the \textsc{Update} operation the same way we treat a \textsc{Find}, as neither operation changes the presence of a key.
...
We do not need to change the templates for either the \textsc{Insert} or \textsc{Delete} operations as shown in Algorithms~\ref{alg:listinsert} and~\ref{alg:listdelete} as that operation does not examine the value associated with a key.

\begin{algorithm}[t]
	\SetKwFunction{UpdateInfoExt}{UpdateInfoExt}
	\SetKwProg{Function}{Function}{}{end}
	\Function{\UpdateInfo{$\textbf{NodeInfo*}\;info,\;$ $\textbf{bool}\;wantkey$}}{
		$\textbf{NodeInfo*}\; oldinfo \gets n.info$\;
		\If{$\textsc{IsMarked}(oldinfo)$}{
		...
		$\textbf{int}\; descValue \gets oldinfo.desc.ops[oldinfo.opid].value$\;
		\If{$op.type == Update$}{
			\If{$descValue \;!= \;node.value \;\&\& \;oldinfo.desc.status == Committed\; \&\&\; IsValuePresent(oldinfo)$}{
				$node.value \gets descValue$\;
			}
		}
	...
	\caption{Map: Extended Update NodeInfo}
...
%{
%	if (nodeDesc->desc->ops[nodeDesc->opid].value != val && nodeDesc->desc->status == MAP\_COMMITTED)
%	if (nodeDesc->desc->ops[nodeDesc->opid].type == MAP\_UPDATE || 
%	(nodeDesc->desc->ops[nodeDesc->opid].type == MAP\_FIND && nodeDesc->desc->ops[nodeDesc->opid].value != 0) )
...
Both the micro-benchmark and the data structure implementations are compiled with GCC 4.7 with C++11 features and \texttt{O3} optimizations.~\footnote{All source code can be downloaded from \url{http://cse.eecs.ucf.edu/gitlab/pierrelaborde/lftt}}
