...
\title{Lock-free Transaction without Rollbacks for Linked Data Structures}
...
\begin{abstract}
    ...
    Although this reduces programming effort, it leads to overhead associated with additional synchronization and the need to rollback failed transactions.
    % TODO: Add 1-2 sentences and explain how our approach is different
    
    In this work, we present a new methodology for transforming high-performance lock-free linked data structures into high-performance lock-free transactional data structures without revamping the data structures' original synchronization design.
    ...
    For each data structure operation, we introduce a procedure that allows the interpretation of the logical status of a node based on the transaction status. 
    Write operations are invisible to operations outside the scope of the transaction until the transaction commits.
    Our approach leverages the semantic knowledge of the data structure implementation to eliminate the overhead of false conflicts and rollbacks.
    In our experimental evaluation using transactions with randomly generated operations, our lock-free transactional lists and skiplist outperform the transactional boosted ones by $40\%$ on average and as much as $125\%$ for large transactions. Furthermore, our methodology outperforms the alternative STM-based approaches by a factor of 3 to 10 across all scenarios. 
    % TODO: Add more details: e.g. higher ratio of successfully executed transactions, other contributions
\end{abstract}
...
\label{sec:intro}
% TODO: include a defense against reviewers complaining about the limitation to linked data structures
...
In this paper, we refer to a transaction as sequence of linearizable operations on a concurrent data structure~\footnote{This can be seen as a special case of memory transactions where the granularity of operation is on the data structure level instead of memory word level.}.
...
Software transactional memory (STM)~\cite{shavit1997software,herlihy2003software} can be used to easily construct transactional data structures from their sequential counterparts: operations executed within an STM transaction are guaranteed to be transactional.
...
We focus our discussion here on the data structures that implement the integer set \emph{abstract data type} with three canonical operations \textsc{Insert}, \textsc{Delete}, and \textsc{Find}.
...
%Our approach associates a logical status with each node by embedding a reference to a \emph{transaction descriptor} in them.
...
% TODO: make it clear what do we claim as contribution in each item; maybe by using a phrase to name a contribution and then explaining it; finish with a claim for Speedup and Efficiency and combine the last 2 items there.
...
\begin{itemize}
    ...
    \item we devised an node-based conflict detection scheme that does not rely on any external data structure nor STM, resulting in native transaction support in stand-alone data structures. No previous approaches achieve this. This allows the transformed data structure to be distributed and integrated  
    \item Our logical status interpretation draws inspiration from logical deletion and extends the idea to enable transaction execution without explicitly revoking aborted transactions.
    ...
    \item We conduct thorough performance evaluation with various transaction sizes and workloads to characterizes the performance of transformed data structures; and recognize their appeal for transactions with more than 8 operations. 
    \item Our approach enforces serialization through cooperative execution, thus the number of aborts caused by node access conflict is brought down to a minimum.
\end{itemize}
...
%We focus on the lock-free case because the obstruction-free version can be trivially obtained from disabling the help mechanism in the lock-free version.
...
\begin{algorithm}[th]
    ...
    \begin{algorithmic}[1]
        \Function{IsNodeExist}{$\textbf{Node*}\; n,\; \textbf{int}\; key$}
        \State \Return $n.key = key$ \label{l:nodeexist}
        \EndFunction
        \Statex
        \Function{IsKeyExist}{$\textbf{NodeInfo*}\;info$}
...
\begin{algorithm}[t]
    ...
    \begin{algorithmic}[1]
        ...
        \EndIf
        \State $\textbf{bool}\;haskey \gets \Call{IsKeyExist}{oldinfo}$ \label{l:testkey} 
...
Given a node $n$, the function \textsc{UpdateInfo} reads its current $info$ field (line~\algref{alg:updateinfo}{l:readinfo}), verifies its sanity, and attempts to update $n.info$ through the use of CAS (line~\algref{alg:updateinfo}{l:casinfo}).
...
We validate the presence of the key on line~\algref{alg:updateinfo}{l:testkey} and test if the key's presence (as required by deletions and finds), or lack of presence (as required by insertions), is desired on line~\algref{alg:updateinfo}{l:testdesire}.
...
Once all operations complete successfully we atomically update the transaction status with a \emph{Commited} flag (line~\algref{alg:transaction}{l:txnsuccess}).
...
A deadlock generates cyclic dependencies among transaction executions, which can be detected by check for duplicate entries in the help stack (line~\algref{alg:transaction}{l:stackcontain}). 
...
The function still resembles the original node insertion algorithm with a CAS-based while loop (line~\algref{alg:listinsert}{l:listinsertwhile}).
The only addition is the code path for invoking \textsc{UpdateInfo} on line~\algref{alg:listinsert}{l:insupdateinfo}, without which they are functionally equivalent.
The logic is very simple: on line~\algref{alg:listinsert}{l:listinsertcontain} we check if the data structure already contains a node with the target key.
...
\begin{algorithm}[t]
    ...
    \begin{algorithmic}[1]
        ...
        \If{\Call{IsNodeExist}{$curr,\;key$}} \label{l:listinsertcontain}
...
Linearizability requires that concurrent operations appear as if they took place instantaneously at some point between their invocations and responses.
...
\begin{proof}
    ...
    Note that if the operation reports failure on line~\algref{alg:updateinfo}{l:infofail1} and~\algref{alg:updateinfo}{l:infofail2}, no write operation is performed to change the logical status of the node.
...
\begin{proof}
    ...
    If another transaction $T_2$ were to access $n_1$, it must perform \textsc{ExecuteOps} for $T_1$ on line~\algref{alg:updateinfo}{l:helpops}.
    Because \textsc{ExecuteOps} always update the transaction status when return on line~\algref{alg:transaction}{l:txnsuccess} or~\algref{alg:transaction}{l:txnfail} (note that failed CAS also means the transaction status has been set, but another thread).
...
\end{theorem}
\begin{proof}
Follow the conclusion in~\cite{herlihy2008transactional}, and Lemma~\ref{lemma:linearizable},~\ref{lemma:commute}, the theorem holds.
\end{proof}
...
Besides the performance advantages, our approach decreases superfluous aborts to a minimum, which is desirable because transaction success rate is a decisive factor for a majority of the applications.
