...
This allows us to preserve the current value of a node, and buffer pending updates in the node's descriptor.
...
The only change we make to the templates for the \textsc{Insert} and \textsc{Delete} operations as shown in Algorithms~\ref{alg:listinsert} and~\ref{alg:listdelete} is that we must set the \textsc{value} field of the new node to the value that is associated with the key, and both algorithms must search for a node using the hashed key instead of the key itself.
