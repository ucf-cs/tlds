...
\begin{algorithm}[t]
    ...
    %\begin{algorithmic}[1]
        \SetKwFunction{UpdateInfo}{$\textbf{Node*}\;n,\;\textbf{NodeInfo*}\;info,\;$ $\textbf{bool},\;wantkey$}
         $\textbf{NodeInfo*}\; oldinfo \gets n.info$ \label{l:readinfo}
        $\textbf{if}$ $IsMarked(oldinfo)$ $\textbf{then}$
         $Do\_Delete(n)$ 
         \KwRet{ retry}
        %endif
        $\textbf{if}$  {$oldinfo.desc \neq info.desc$} \label{l:testself} $\textbf{then}$
         ExecuteOps($oldinfo.desc,\;oldinfo.opid + 1$) \label{l:helpops}
        $textbf{else if}$  {$oldinfo.opid \ge info.opid$} \label{l:testhelped} $\textbf{then}$
         \KwRet{ success}
        %endif
         $\textbf{bool}\;haskey \gets IsKeyPresent(oldinfo)$ \label{l:testkey} 
        $\textbf{if}$ { $(!haskey \;\textbf{and}\; wantkey) \;\textbf{or}\; (haskey \;\textbf{and}\; !wantkey)$} \label{l:testdesire}} $\textbf{then}$
         \KwRet{ fail \label{l:infofail1}}
        %endif
        $\textbf{if}$ { {$info.desc.status \neq Active$} \label{l:testactive} $\textbf{then}$
         \KwRet{ fail \label{l:infofail2}}
        %endif
        $\textbf{if}$  {$CAS(\&n.info,\;oldinfo,\;info)$}  $\textbf{then}$ \label{l:casinfo}
         \KwRet{ success}
        $textbf{else}$
         \KwRet{ retry}
        %endif
...
\begin{algorithm}[th]
    ...
    %\begin{algorithmic}[1]
        ...
        \SetKwFunction{ExecuteTransaction}{$\textbf{Desc*}\;desc$}
         $helpstack.\textbf{Init}$
         $ExecuteOps(desc, 0)$
         \KwRet{ $desc.status = Committed$}
        
        %x
        \SetKwFunction{ExecuteOps}{$\textbf{Desc*}\;desc,\;\textbf{int}\;opid$}
         $\textbf{bool}\;ret \gets \textbf{true}$
         $\textbf{set}\;delnodes$
        $\textbf{if}$  {$helpstack.Contains(desc)$}  $\textbf{then}$ \label{l:stackcontain}
         $\text{\textsc{CAS}}(\&desc.flag,\;Active,\;Aborted)$ \label{l:txncyclicabort}
         \KwRet{}
        %endif
         $helpstack.Push(desc)$
         \label{l:txnwhile} %TODO: make sure this ends up on the while
        ...
        \While {$desc.status = Active$ $\;\textbf{and}\; ret \;\textbf{and}\;$ $opid < desc.size$} 
         {
	        ...
	        $\textbf{if}$ {{$op.type = \textbf{find}$} } $\textbf{then}$
	         $ret \gets Find(op.key,\;desc,\;opid)$
	        $textbf{else if}$ { {$op.type = \textbf{insert}$}} $\textbf{then}$
	         $ret \gets Insert(op.key,\;desc,\;opid)$
	        $textbf{else if}$ {{$op.type = \textbf{delete}$}} $\textbf{then}$
	         $\textbf{Node*}\;del$
         ...
         $helpstack.\textbf{Pop}$
        $\textbf{if}$ { {$ret = \textbf{true}$}} $\textbf{then}$
        $\textbf{if}$ { {$\text{\textsc{CAS}}(\&desc.flag,\;Active,\;Committed)$} \label{l:txnsuccess}} $\textbf{then}$
...
Our helping mechanism invokes a slight memory and performance overhead, due to the use and maintenance of the per-thread help stack.
In situations of low contention, it may be beneficial to eliminate this overhead by disabling the helping mechanism.
We introduce an alternative obstruction-free approach that disables the helping mechanism, and we compare it to our original lock-free approach.
...
\begin{algorithm}[t]
    ...
    %\begin{algorithmic}[1]
        \SetKwFunction{Insert}{$\textbf{int}\;key,\;\textbf{Desc*}\;desc,\;\textbf{int}\;opid$}
         $\textbf{NodeInfo*}\; info \gets \text{\textbf{new NodeInfo}}$
         $info.desc \gets desc,\;info.opid \gets opid$
         \label{l:listinsertwhile} %TODO: make sure this hits the while
         \SetKwProg{While}{while}{}{}
        \While{\textbf{true}} 
        {
	         $\textbf{Node*}\;curr \gets Do\_LocatePred(key)$
	        $\textbf{if}$ (IsNodePresent($curr,\;key$)) $\textbf{then}$ \label{l:listinsertcontain}
	        ...
	        %endif
	        $\textbf{if}$ { {$ret = success$}} $\textbf{then}$
	         \KwRet{ \textbf{true}}
	        $textbf{else if}$ { {$ret = fail$}} $\textbf{then}$
	         \KwRet{ \textbf{false}}
	        %endif
	    }
...
\begin{algorithm}[t]
    ...
    %\begin{algorithmic}[1]
        \SetKwFunction{Find}{$\textbf{int}\;key,\;\textbf{Desc*}\;desc,\;\textbf{int}\;opid$}
         $\textbf{NodeInfo*}\; info \gets \text{\textbf{new NodeInfo}}$
         $info.desc \gets desc,\;info.opid \gets opid$
         \label{l:listinsertwhile} %TODO: make sure this hits the while
         \SetKwProg{While}{while}{}{}
        ...
        {
	         $\textbf{Node*}\;curr \gets Do\_LocatePred(key)$
	        $\textbf{if}$ (IsNodePresent($curr,\;key$)) $\textbf{then}$
	         $ret \gets UpdateInfo(curr,\;info,\;true)$
	        $	extbf{else}$
	         $ret \gets fail$
	        %endif
	        $\textbf{if}$ { {$ret = success$}} $\textbf{then}$
	         \KwRet{ \textbf{true}}
	        $textbf{else if}$ { {$ret = fail$} \label{l:findfail} } $\textbf{then}$
	         \KwRet{ \textbf{false}}
	        %endif
	    }
...
\begin{algorithm}[h]
    ...
    %\begin{algorithmic}[1]
        \SetKwFunction{Delete}{$\textbf{int}\;key,\;\textbf{Desc*}\;desc,\;\textbf{int}\;opid,\;$ $\textbf{ref Node*}\; del$}
         $\textbf{NodeInfo*}\; info \gets \text{\textbf{new NodeInfo}}$
         $info.desc \gets desc,\;info.opid \gets opid$
          \label{l:listinsertwhile} %TODO:make sure this ends up on the while
         \SetKwProg{While}{while}{}{}
        \While{\textbf{true}}
        {
	         $\textbf{Node*}\;curr \gets Do\_LocatePred(key)$
	        ...
	        $\textbf{if}$ { {$ret = success$}} $\textbf{then}$
	         $del \gets curr$
	         \KwRet{ \textbf{true}}
	        $textbf{else if}$ { {$ret = fail$} \label{l:deletefail}} $\textbf{then}$
	         $del \gets NIL$
	         \KwRet{ \textbf{false}}
	        %endif
	    }
        ...
        %x
        \SetKwFunction{MarkDelete}{$\textbf{set}\;delnodes,\;\textbf{Desc*}\;desc$}
        \SetKwProg{For}{for}{}{}
        \For {$del \in delnodes$}
        {
	        $\textbf{if}$ ($del = NIL$) $\textbf{then}$
	         \textbf{continue}
	        %endif
	        ...
	        $\textbf{if}$ ($info.desc \ne desc$) $\textbf{then}$
	         \textbf{continue}
	        %endif
	        $\textbf{if}$ ($CAS(del.info,info,\textsc{SetMark}(info))$)  $\textbf{then}$ \label{l:markinfo}
	         $Do\_Delete(del)$
	        %endif
	    }
...
In Figure~\ref{fig:obslist}, we show the throughput and the number of spurious aborts for the LFT list and the OFT list.
...
In Figure~\ref{fig:obsskip}, we compare the performance of the OFT skiplist to the LFT skiplist.
