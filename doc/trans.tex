...
\begin{abstract}
    ...
    Our methodology outperforms the alternative STM-based approaches by a factor of 3 to 10 across all scenarios. 
    More importantly, we achieve 4 to 6 orders of magnitude less spurious conflicts than the alternatives.
\end{abstract}

\category{D.1.3}{Concurrent Programming}{Algorithms}
%\terms{Algorithms, Performance}
\keywords{
    Transactional Data Structure, Transactional Memory, Lock-free 
}
...
\begin{itemize}
    ...
    \item We introduce a node-based conflict detection scheme that does not rely on STM nor require the use of an additional data structure. This enable us to augment linked data structures with native transaction support.
...
%TODO: briefly explain how do we arrive at the transformed data structure
...
In our transformed data structures, conflicts are detected at the granularity of a node. 
...
% TODO: say something like: In case of cyclic dependencies among transactions, deadlock can occur. To prevent this, our methodology implements this and that ...
% TODO: do not use deadlock
...
% TODO: Rephrase these two sentences: just say: In order to guarantee lock-free progress, the later transaction must help carry ...
...
In the second case, the data structure becomes obstruction-free, in the last case it is lock-free.
...
Furthermore, node-based conflict detection effectively compartmentalizes the execution of transactions.
...
We achieve this by having operations \emph{inversely interpret} the logical status of nodes accessed by operations in a failed transaction.
Both physical undo and our logical undo reach the same goal of restoring the abstract state of the data structure. 

% TODO: Replace IsNodeExist, IsKeyExist with NodePresent, KeyPresent
...
Given a node's presence, function \textsc{IsKeyExist} verifies if the key should be logically included in the set, and returns a boolean value based on the combination of operation type and transaction status.
...
This is a refactoring process, as we do not alter the functionality of the original implementations. 
...
\textsc{Do\_Delete} should remove any references to the node.
...
The function resembles the original node insertion algorithm with a CAS-based while loop (line~\algref{alg:listinsert}{l:listinsertwhile}).
