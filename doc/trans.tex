...
\begin{abstract}
    ...
    We coordinate threads to help finish the remaining operation of delayed a transaction based on its transaction descriptor.
    For each data structure operation, we introduce an additional procedure that allows them to interpret the logical status of a node based on the transaction status. 
    Write operations are invisible to operations outside the scope of the transaction until the transaction commits.
    Our approach leverages the semantic knowledge of the data structure implementation to eliminate the overhead of false conflict and rollback.
    ...
    STM based approaches fall behind by as much as 3~10 times across all scenarios.
\end{abstract}
...
Lock-free transactional transformation treats the based data structure as a \emph{white box}, and introduces a new code path for transaction level synchronization using only the single-word \textsc{CompareAndSwap} (CAS) synchronization primitive.
The two key challenges for high-performance data structure transaction executions are: 1) to efficiently buffer write operations so that its modification is invisible to operations outside the transaction scope; and 2) to minimize the penalty of rollbacks when aborting partially executed transactions. 
...
When an operation tries to access a node, it first reads the node's descriptor and only proceed with its modification if the descriptor indicates the previous transaction has committed or aborted. 
Otherwise, the operation help execute active transaction according to the instructions in the descriptor.
...
This process interpret the logical status of the nodes left behind by an aborted transaction in such a way that concurrent operations observe a consistent abstract state as if the aborted transaction has been revoked.
...
Usually the mapping is simple---every node in the concrete state correspond to an key in the abstract state.
Previous works on lock-free data structure has been using \emph{logical deletion}~\cite{harris2001pragmatic}, in which a key is considered removed from the abstract state if the corresponding node has been bit-marked.
Logical deletion encodes a binary logical status so that a node only maps to a key if its reference has not been bit-marked.
We further generalize this technique by interpreting a node's logical status based on the combination of transaction status and operation type.
...
We execute a micro-bench on a 64-core NUMA system to measure the throughput and number of aborts under three types of workloads.
The result shows that our transactional data structure achieve an average of $40\%$ speedup over transactional boosting based approaches, an average of 10 times speedup over the word-based STM, and an average of 3 times over the object-based STM.
Moreover, the aborts generated by our approach is 4 orders of magnitude less than transactional boosting and 6 orders of magnitude less than STMs.

This paper makes the following contributions:
\begin{itemize}
    \item To the best of our knowledge, lock-free transactional transform is the first methodology that provides lock-free progress for data structure transactions. 
    ...
    \item Because our transaction level synchronization is tightly integrated with the base data structure's thread level synchronization, we are able to exploit the considerable amount of effort and brilliance devoted to the development lock-free data structures.
    \item We conduct thorough performance evaluation with various transaction sizes and workloads to characterizes the performance of transformed data structures; and recognizes their appeal for transactions with more than 8 operations. 
...
We present the template for building transactional linked list and skiplist in Section~\ref{sec:application}.
...
\begin{algorithm}[th]
    ...
    \begin{algorithmic}[1]
        ...
        \State $\textbf{TxStatus}\;status \gets info.desc.status$
        \Switch{$status$} 
        \Case{$\ACTIVE$}
        \EndCase
        \Case{$\COMMITTED$} 
...
\begin{algorithm}[t]
    ...
    \begin{algorithmic}[1]
        ...
        \State $ret \gets \Call{Do\_Insert}{n}$
        \EndIf
        \If {$ret = success$}
        \State \Return \TRUE
        \ElsIf {$ret = fail$}
